function _add_components_to_ref!(ref::Dict{Symbol,Any}, data::Dict{String,Any})
    ...
    for (i, receipt) in get(data, "receipt", [])
        ...
        ref[name][id]["offer_price_NG"] = receipt["offer_price_NG"]
        ref[name][id]["offer_price_H2"] = receipt["offer_price_H2"]
    end
    ...
    for (i, delivery) in get(data, "delivery", [])
        ...
        ref[name][id]["bid_price_MJ"] = delivery["bid_price_MJ"]
...
function _update_pipe_fields!(ref::Dict{Symbol,Any}, params, nominal_values, data::Dict{String,Any})
    for (_, pipe) in ref[:pipe]
        ...
        pipe["flow_min"] = pipe["flow_min"] < 0 ? 0 : pipe["flow_min"]
    end
...
function build_ref(data::Dict{String,Any},
    ...
    ref[:multiplier] = (nominal_values[:density] * nominal_values[:velocity] / nominal_values[:pressure])^2
    ref[:nominal_massflow] = nominal_values[:mass_flow]
    ref[:nominal_pressure] = nominal_values[:pressure]
